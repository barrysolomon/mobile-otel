...
- [x] **iOS: log records exported twice** — fixed 2026-04-23. Root cause: `OTelMobile.swift` registered both `bufferProcessor` AND `BatchLogRecordProcessor` on the same `LoggerProvider` via `with(processors: [bufferProcessor, batchLogProcessor])` — each processor owned its own export pipeline, so every emit produced two OTLP POSTs. Confirmed via Dash0 CLI on `otel-rn-astronomy-shop` showing every `app.recovery_start` (and previously `cart.add_item` / `shop.view_product`) landing twice at identical timestamps. Fix: dropped `batchLogProcessor` entirely (Android model — Android's `MobileLoggerProvider` only registers `mobileProcessor`) and added `MobileLogRecordProcessor.startContinuousFlush(intervalSeconds:)` so CONTINUOUS mode still gets periodic log export. New `MobileConfig.logExportIntervalSeconds` field (default 30, matches Android's `traceExportIntervalSeconds`). Also fixed a pre-existing `EndToEndSmoke.endToEndEmitFlushCapture` flake along the way — it asserted pairwise sequenceId ordering, but `onEmit` appends via `Task.detached` which has no FIFO guarantee; relaxed to set-equality. Host tests 287/287 (includes 3 new timer tests: periodic flush fires, stop halts firing, double-start replaces previous timer).
- [x] **RN bridge: span durations show 0ms for most child spans** — fixed 2026-04-24. Root cause was worse than the backlog note suggested: `Date.now() * 1_000_000` didn't just lose sub-ms fidelity, it also lost precision past `Number.MAX_SAFE_INTEGER` (2^53). Unix nanoseconds in 2026 are ~1.77e18 (19 digits); JS doubles can represent ~9e15 (16 digits), so every timestamp rounded to ~128-nanosecond bands. Nested spans within the same band collapsed to duration=0. Fix: `nowUnixNano` now captures `epochAnchorMs = Date.now()` and `perfAnchorMs = performance.now()` on first call, then returns `BigInt(epochAnchorMs) * 1_000_000n + BigInt(Math.floor((performance.now() - perfAnchorMs) * 1_000_000))` as a string. `performance.now()` on RN 0.85 gives sub-ms resolution via JSI; graceful fallback to `Date.now()` when unavailable. BigInt arithmetic preserves every digit. On-device validated on otel-rn-astronomy-shop: 23 spans landed, **0 zero-duration** (was ~70% pre-fix); nested children show 1-10µs, root `checkout` shows 3.8ms, real HTTP `GET httpbin.org` shows 646ms. Single-bridge fix — both Android + iOS consumers benefit since JS is the source of timestamp truth. 116/116 Jest tests, 5 new dedicated precision tests, tsc clean.
