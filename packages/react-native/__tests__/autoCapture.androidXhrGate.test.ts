 ...
 *
 * The module graph is loaded inside `jest.isolateModulesAsync` with a fresh,
 * Android-flavored `react-native` mock so this file's virtual mock cannot leak
 * into (or be defeated by) other suites sharing a worker's module registry.
 */
...
describe('Android JS XHR shim gate', () => {
  ...
  it('on Android, network=true installs NEITHER the JS XHR shim NOR the fetch shim (native owns it)', async () => {
    await jest.isolateModulesAsync(async () => {
      // Fresh, isolated module graph with an Android-flavored react-native so
      // index.ts's `require('react-native').Platform.OS` resolves to 'android'.
      jest.doMock('react-native', () => ({ Platform: { OS: 'android' } }), { virtual: true });

      // eslint-disable-next-line @typescript-eslint/no-var-requires
      const mod = require('../src') as typeof import('../src');
      const { Dash0Mobile, __setNativeForTesting, __resetForTesting } = mod;
