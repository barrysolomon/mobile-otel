...
- **Stability:** **GA / stable (`1.0.0`)** — the first generally-available release; the full [SemVer 2.0](https://semver.org/) guarantee is now in force. Un-annotated public symbols are **stable** (breaking change ⇒ MAJOR bump + deprecation cycle); symbols annotated `@Incubating` (Kotlin) / documented as experimental (Swift/TS) may still change in a MINOR — see the tier table below and [API_STABILITY.md](API_STABILITY.md).
...
This section originally recommended **against** cutting 1.0 at `0.2.0-alpha`, behind five gates. Status at the `0.9.0-beta` cut (all five gates green; 1.0 was deferred only for a post-release CI soak, not for any open gate). **That soak is complete and `1.0.0` GA is now cut (2026-07-14).**
