...
- [ ] **Android emulator → Dash0 ingress IP routing** (surfaced 2026-04-24). Pixel_7 AVD on Barry's machine resolves DNS and reaches `8.8.8.8` but cannot reach `35.160.215.182` (current Dash0 ingress IP). Host machine + iOS Simulator both reach the same IP fine, so this is per-emulator NAT/egress config, not infrastructure. Blocks Android native + RN Android matchy-matchy validation. Workarounds documented in [`docs/matchy-matchy/android-native.md` §0](../matchy-matchy/android-native.md#-environment-limitation-discovered-2026-04-24): try `-dns-server 8.8.8.8` on emulator boot, swap to a different AVD, build with `upstream` flavor against a local collector forwarder. Revisit when an emulator with working egress is available or when one of the workarounds lands a clean run.
- [ ] **RN iOS Gate 1 — AppState init race** (re-investigated 2026-04-24, still 🔴). Both 100ms and 1500ms JS-side defers redbox under RN 0.85 + iOS 26.4 + new-arch + Hermes. The `Invariant Violation` from `TurboModuleRegistry.getEnforcing('PlatformConstants')` fires before our deferred callback runs and is converted by RN's `RCTFatal` into a native fatal that bypasses our JS try/catch. Upstream RN issue, not defer-tuning. See [matchy-matchy/rn-ios.md "Gate 1 unblock investigation"](../matchy-matchy/rn-ios.md#1-gate-1--lifecycle-) for full diagnosis + future-attempt ideas (rAF + setTimeout(0) chain; native-side lifecycle riding through bridge; await upstream fix).
