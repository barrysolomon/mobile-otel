...
The SDK ships as a Swift Package with ten library products (the package itself is named `OTelMobile` and lives in `otel-ios-mobile/` at the repo root). Pin the shipped tag `v0.5.2-alpha` in your app's `Package.swift`:

```swift
dependencies: [
    .package(url: "https://github.com/barrysolomon/mobile-otel.git", exact: "v0.5.2-alpha"),
...
| `telemetry.sdk.version` | `"0.5.2-alpha"` — the literal in [ResourceBuilder.swift](../otel-ios-mobile/Sources/OTelMobileSDK/Resource/ResourceBuilder.swift) (`sdkVersion`), matching the shipped package tag `v0.5.2-alpha`. |
