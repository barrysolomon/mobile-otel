...
The SDK ships as a Swift Package with ten library products (the package itself is named `OTelMobile` and lives in `otel-ios-mobile/` at the repo root). Pin the shipped tag `v0.2.1-alpha` in your app's `Package.swift`:
...
| `telemetry.sdk.version` | `"0.2.1-alpha"` — the literal in [ResourceBuilder.swift](../otel-ios-mobile/Sources/OTelMobileSDK/Resource/ResourceBuilder.swift) (`sdkVersion`), matching the shipped package tag `v0.2.1-alpha`. |
