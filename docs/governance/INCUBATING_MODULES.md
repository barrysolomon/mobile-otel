...
| Database | OTel-native but limited validation | Real-world Room query validation, slow-query threshold tuning | 0.4.0-alpha |
| File I/O | OTel-native but limited validation | Real-world file operation profiling, performance impact measurement | 0.4.0-alpha |
| System Events | OTel-native but battery impact unknown | Battery drain measurement on 3+ device models | 0.4.0-alpha |
| Timber | OTel-native, niche dependency | Validate graceful no-op when Timber not on classpath | 0.4.0-alpha |
| Amplify DataStore | OTel-native, niche dependency | Innovapptive beta validation, Amplify version matrix | Customer-driven |
| Screen Orientation | OTel-native, minor feature | Bundle with next GA release | 0.4.0-alpha |
