...
### Release engineering (0.9.0-beta)

- **Fixed: iOS simulator device-tests deadlocked 8 offline/network suites on hosted runners (#66).** `MobileLogRecordProcessor.forceFlushBuffered()` parked its calling thread on a `DispatchSemaphore` while the drain ran in a detached Task — which needs a slot on the *same* width-limited cooperative executor. Swift Testing runs suites in parallel; on a 3-core CI runner enough concurrent async callers parked every executor thread, the detached tasks could never run, and every in-flight suite froze until the 90-minute job timeout (many-core dev machines never saturate, hence "passes locally"). The drain logic now lives in an async core (`forceFlushBufferedAsync()`); the sync bridge remains only for libdispatch callers (OTel protocol surface, CONTINUOUS timer, lifecycle auto-flush), and the network-restored hook plus all async test call sites use the async path. Guards added: `ios-ci.yml` runs the buffering/policy suites per-push under `LIBDISPATCH_COOPERATIVE_POOL_STRICT=1` (any executor-starvation regression hangs deterministically and fails with a named error), and the simulator job now passes per-test time allowances to `xcodebuild` so a future hang becomes a named test failure instead of an opaque job timeout. Public API addition: `OTelMobile.forceFlushAsync()`, the async twin of `forceFlush()` for Swift-concurrency callers. See docs/contracts/buffer-drain-surface.md.
...
- **Docs**: install snippets and current-version references across README, tutorials, and platform guides bumped from 0.5.2-alpha to 0.9.0-beta; obsolete `@alpha` npm dist-tag install advice replaced with plain (`latest`) installs — the publish workflow's pre-1.0 exception points `latest` at every 0.x release, so a bare `npm install` resolves to this beta.

### Added

- **`symbol-upload` CLI — symbolication Phase 2** (`tools/symbol-upload/`, pure-Go stdlib): pushes crash-symbolication mappings to a store keyed by the **same build id the SDK stamps into `app.build.id`** (Phase 1), closing the loop so a minified crash matches its mapping. One binary with per-platform adapters — **Android** `mapping.txt` keyed by `io.dash0.mobile.BUILD_ID`, **iOS** dSYM keyed by the Mach-O `LC_UUID` read out of the bundle (one upload per UUID), **RN** source-map keyed by the JS bundle's SHA-256 (printed for `Dash0Mobile.start({ buildId })`). Content-addressed by `(platform, build-id)` with a `HEAD`-before-`PUT` idempotency check, so it is safe on every CI build; gzip-compressed uploads, bearer auth + optional `Dash0-Dataset`. New `symbol-upload-tool` CI job runs `go test -race` + `go vet`. Build-system wiring (Gradle task, Xcode Run Script, Metro step) in [tools/symbol-upload/README.md](tools/symbol-upload/README.md). Backend symbolicator (Phase 3) is a separate Dash0 track.
- **`app.build.id` resource attribute — symbolication Phase 1** (docs/design/symbolication.md, first NEXT-tier roadmap item): every signal now carries the identifier that matches a crash/error to the artifact that symbolicates it. **iOS** derives the main executable's Mach-O `LC_UUID` at runtime (`BuildIdReader`) — no build tooling required, it matches the dSYM by construction. **Android** reads a build-time manifest stamp (`<meta-data android:name="io.dash0.mobile.BUILD_ID">`, typically a per-build UUID via `manifestPlaceholders`) that keys the R8 `mapping.txt` upload. **React Native** adds `Dash0Mobile.start({ buildId })` for the JS bundle / source-map id. On all platforms `extraResourceAttributes["app.build.id"]` (RN: the dedicated field) overrides. Unstamped Android apps simply omit the attribute.
...
- **Project identity clarified — independent OpenTelemetry-native SDK (N15)**: resolved the OpenTelemetry/CNCF misattribution. Copyright is now **Barry Solomon** across NOTICE, CONTRIBUTING, and all 376 first-party source headers (`Copyright The OpenTelemetry Authors` → `Copyright 2025 Barry Solomon`). NOTICE and README now carry an explicit affiliation/trademark disclaimer (not an official OpenTelemetry project; "OpenTelemetry" is a trademark of The Linux Foundation, used only to describe compatibility). CONTRIBUTING's title, code-of-conduct, license-header snippet, and community channels were de-CNCF'd (Issues/Discussions instead of OTel Slack / CNCF mailing list / SIG). Files genuinely derived from the upstream OpenTelemetry community demo (`examples/upstream-demo-app*`) **retain** their original OTel Authors notices, per Apache-2.0 §4(c).
- **Documentation truth-up (N16)**: reconciled stale status docs against shipped reality — UAT matrix marked 48/48 green (all phases complete); SCALE_READINESS epic marked COMPLETE (SR-001–006/014 shown shipped via PR-001–006, resolving the contradiction with PRODUCTION_READINESS); ios-parity-epic banner'd SUPERSEDED; IOS_ANDROID_PARITY tap/scroll/text-input corrected ❌→✅ (verified in the iOS SwiftUI modifier sources) plus the "no iOS docs" claim fixed; BACKLOG Track 7/8 (upstream convergence, iOS port, RN bridge, symbolication Phases 1–2) and the SR checkboxes flipped to done; all three battle cards updated (iOS SDK, React Native, and crash symbolication now ship). Genuinely-open items (WebSocket/AndroidLog modules, session-replay viewer, APM correlation, NDK, server-side symbolication) left open.
- **README discoverability (N18)**: added a "Stability & Versioning" documentation subsection linking `API_STABILITY.md`, `VERSIONING.md`, and `FEATURE_MATURITY_MATRIX.md` (previously zero inbound links).
