...
class AmplifyDataStoreInstrumentationTest {
    ...
    @Test fun `outboxMutationEnqueued emits log`() {
        ...
        val logs = otelRule.logRecords.filter { it.body.asString() == MobileSemconv.DATASTORE_OUTBOX_ENQUEUED }
    ...
    @Test fun `outboxMutationProcessed emits log`() {
        ...
        val logs = otelRule.logRecords.filter { it.body.asString() == MobileSemconv.DATASTORE_OUTBOX_PROCESSED }
        assertTrue(logs.size == 1)
    }
    ...
    @Test fun `captureOutboxEvents=false suppresses outbox`() {
        ...
        fireHubEvent(inst, "outboxMutationEnqueued")
        fireHubEvent(inst, "outboxMutationProcessed")
        val outboxLogs = otelRule.logRecords.filter { it.body.asString().startsWith("datastore.outbox") }
    ...
    @Test fun `captureSyncMetrics=false suppresses modelSynced`() {
        ...
        fireHubEvent(inst, "modelSynced")
        assertTrue(otelRule.logRecords.filter { it.body.asString() == MobileSemconv.DATASTORE_MODEL_SYNCED }.isEmpty())
    }
    ...
    @Test fun `attachNetworkState=false suppresses networkStatusChanged`() {
        ...
        fireHubEvent(inst, "networkStatusChanged")
        assertTrue(otelRule.logRecords.filter { it.body.asString() == MobileSemconv.DATASTORE_NETWORK_CHANGED }.isEmpty())
    }
