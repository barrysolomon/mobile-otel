...
/**
 * DirectionsFragment — demonstrates how to add a custom OTel transaction (span) that
 * wraps multiple steps: location lookup → office search (HTTP) → route fetch (HTTP).
 *
 * ─────────────────────────────────────────────────────────────────────────────
 * HOW TO ADD YOUR OWN TRANSACTION
 * ─────────────────────────────────────────────────────────────────────────────
 *
 *  1. Start a span when the user action begins:
 *       val span = OTelMobile.getTracer("your.component")
 *           .spanBuilder("user.action_name")
 *           .setAttribute(...)
 *           .startSpan()
 *       val scope = span.makeCurrent()   // makes this the "current" span on this thread
 *
 *  2. Add events at key points to mark progress:
 *       span.addEvent("step.completed", Attributes.of(...))
 *
 *  3. When crossing thread boundaries (Dispatchers.IO), restore the OTel context so
 *     child spans (e.g., HTTP calls via OTelNetworkInterceptor) are linked correctly:
 *       val ctx = Context.current()            // capture on main thread
 *       withContext(Dispatchers.IO) {
 *           val ioScope = ctx.makeCurrent()    // restore on IO thread
 *           try { ... } finally { ioScope.close() }
 *       }
 *
 *  4. Record errors:
 ...
 *
 *  5. Always end the span — use try/finally or always call span.end():
 *       scope.close()
 *       span.end()
 * ─────────────────────────────────────────────────────────────────────────────
 */
class DirectionsFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        MobileOtel.sendEvent("screen.viewed", mapOf("screen.name" to "DirectionsFragment"), Severity.INFO)
     ...
     *
     * Both calls are automatically traced by OTelNetworkInterceptor as child spans
     * of the parent "user.get_directions" span.
     */
    private fun fetchDirections() {
        setLoading(true)
        tvStatus.text = "Finding nearby offices…"

        // Annotate the current page span (started by AutoCaptureManager) with origin.
        Span.current().setAttribute(AttributeKey.doubleKey("origin.lat"), currentLat)
        Span.current().setAttribute(AttributeKey.doubleKey("origin.lon"), currentLon)

        // Capture context before crossing thread boundary so HTTP child spans are nested
        // under the page span via OTelNetworkInterceptor.
        val ctx = Context.current()

        viewLifecycleOwner.lifecycleScope.launch {
            try {
                val result = withContext(Dispatchers.IO) {
                    val ioScope = ctx.makeCurrent()
                    try {
                        fetchDirectionsOnIo()
                    } finally {
                        ioScope.close()
                ...
                Span.current().addEvent("directions.ready")
                Span.current().setAttribute(AttributeKey.longKey("route.distance_m"), result.distanceMeters)
                Span.current().setAttribute(AttributeKey.longKey("route.duration_s"), result.durationSeconds)
                Span.current().setAttribute(AttributeKey.stringKey("destination.name"), result.officeName)
                Span.current().setStatus(StatusCode.OK)

                setLoading(false)
                showResults(result)

                MobileOtel.sendEvent("directions.fetched", mapOf(
                    "origin.lat"       to currentLat,
                    "origin.lon"       to currentLon,
                    "destination.name" to result.officeName,
                    "route.distance_m" to result.distanceMeters,
                    "route.duration_s" to result.durationSeconds
                ), Severity.INFO)

            } catch (e: Exception) {
                Span.current().recordException(e)
                Span.current().setStatus(StatusCode.ERROR, e.message ?: "directions fetch failed")
                setLoading(false)
                showError(e.message ?: "Could not get directions")
                MobileOtel.reportError(e, mapOf("context" to "directions_fetch"))

            } finally {
                // Close page span with result; start fresh for the next Get Directions tap.
                OTelMobile.restartPageSpan("DirectionsFragment")
            }
        }
    }

    /**
     * Runs on the IO thread. Makes two real HTTP calls that are auto-traced as children
     * of the parent "user.get_directions" span because we restored OTel context before calling.
     */
    private fun fetchDirectionsOnIo(): DirectionsResult {
        ...
        // ── HTTP Call 1: Nominatim search — auto-traced by OTelNetworkInterceptor ──
        // Finds the name of a real place near the random destination point.
        ...
        val nominatimJson = try {
            client.getWithHeader(nominatimUrl, "User-Agent" to "Schedulr-OTel-Demo/1.0")
        } catch (e: Exception) {
            "{}"  // non-fatal — fall back to generic name
        }
        val officeName = parseOfficeName(nominatimJson, destLat, destLon)

        // ── HTTP Call 2: OSRM routing — auto-traced by OTelNetworkInterceptor ───
        // Returns driving distance and duration from current location to the office.
