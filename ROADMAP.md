...
| N7 | 🟡 **PARTIALLY CLOSED 2026-07-03**: verified Android (`policy/RemoteGate.kt` at both choke points) AND iOS (`Policy/RemoteGate.swift` via `ConfigPoller` + `RemoteGatedSampler`) both ship the kill switch; PRR doc row corrected. **Remaining**: RN bridge exposes no gateway/polling options, so RN apps can't enable it; iOS poller off by default | verified in code both platforms | M→S (RN plumbing only) |
...
| N9 | **Fix OPERATIONS_GUIDE/CLAUDE.md transport contradiction** — docs say gRPC :4317 default; SDK default is HTTP/protobuf (`MobileConfig.kt:144`) | OPERATIONS_GUIDE.md:30,38 | S |
...
| N12 | **npm `--provenance` + GH Packages 409 idempotency** — one-flag supply-chain win; make republish re-runs not fail; record both follow-ups in BACKLOG.md (currently only in session memory) | publish.yml:131,188-207 | S |
| N13 | **Fix kiosk-demo dead coordinates** — customer-facing demo pins `io.opentelemetry.android:mobile:0.1.0-alpha` via mavenLocal; can't build clean, demos a 5-versions-old SDK | kiosk-demo/app/build.gradle.kts:124-127 | S |
...
| N14 | **Guard the two unguarded thread-construction sites** | `TapCapture.kt:68`, `FreezeDetector.kt:30` | S |
...
- **Symbolication pipeline** *(tracked, promote to top)*: Phase 1 now — stamp R8 build-ID + Mach-O image UUID as resource attrs (S, unblocks retroactive resolution); then Gradle mapping.txt upload keyed by build ID + collector-side stack rewrite (L); iOS dSYM upload CLI next (L). Unreadable minified production crashes block real adoption. `BACKLOG.md:23`, docs/design/symbolication.md ("the #1 competitive gap").
...
- **RN source-map symbolication** (L, untracked, P0 for RN): release JS stacks ship minified (`errors.ts:69`); need Metro source-map upload tooling + Hermes `.hbc` handling.
