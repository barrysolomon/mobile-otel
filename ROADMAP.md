...
- **Symbolication pipeline** *(tracked, promoted to top)*: ✅ **Phase 1 DONE 2026-07-03** — `app.build.id` resource attr on all 3 platforms (iOS Mach-O `LC_UUID` self-derived via `BuildIdReader`; Android manifest stamp `io.dash0.mobile.BUILD_ID` via `BuildId`+`MobileResource`; RN `start({ buildId })`), TDD'd on each platform; Gradle stamping snippet in docs/design/symbolication.md Phase 1. **Next**: Phase 2 — Gradle mapping.txt upload keyed by build ID + collector-side stack rewrite (L); iOS dSYM upload CLI (L). Unreadable minified production crashes block real adoption. `BACKLOG.md:23`, docs/design/symbolication.md ("the #1 competitive gap").
