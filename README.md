...
> **Current release: `0.3.0-alpha`** — published for Android (GitHub Packages), iOS (SwiftPM), and React Native (npm `@barrysolomon/mobile-react-native@alpha`). See the [Changelog](CHANGELOG.md) for the full 0.3.0-alpha notes.
...
Add the dependency from GitHub Packages (artifact `io.opentelemetry.android:mobile:0.3.0-alpha`). Consuming GitHub Packages requires a GitHub personal access token with the `read:packages` scope:
...
Add the package in Xcode (**File → Add Package Dependencies…**) or in `Package.swift`, pointing at tag `v0.3.0-alpha`, and depend on the `OTelMobileSDK` product:

```swift
// Package.swift
dependencies: [
    // SwiftPM resolves the git tag literally — the release tag is v0.3.0-alpha
    .package(url: "https://github.com/barrysolomon/mobile-otel", .exact("v0.3.0-alpha"))
],
targets: [
