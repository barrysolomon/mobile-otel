...
> **Current release: `0.2.1-alpha`** — published for Android (GitHub Packages), iOS (SwiftPM), and React Native (npm `@barrysolomon/mobile-react-native@alpha`). See the [Changelog](CHANGELOG.md) for the full 0.2.1-alpha notes.
...
Add the dependency from GitHub Packages (artifact `io.opentelemetry.android:mobile:0.2.1-alpha`). Consuming GitHub Packages requires a GitHub personal access token with the `read:packages` scope:
...
Add the package in Xcode (**File → Add Package Dependencies…**) or in `Package.swift`, pointing at tag `v0.2.1-alpha`, and depend on the `OTelMobileSDK` product:

```swift
// Package.swift
dependencies: [
    // SwiftPM resolves the git tag literally — the release tag is v0.2.1-alpha
    .package(url: "https://github.com/barrysolomon/mobile-otel", .exact("v0.2.1-alpha"))
],
targets: [
